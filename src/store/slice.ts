...
const appSlice = createSlice({
    ...
    initialState,
    reducers: {
        selectRestaurant: (state: State, action: PayloadAction<Restaurant | null>) => {
        ...
        removeSelectedItem: (state: State, action: PayloadAction<{index: number, id: string}>) => {
            state.selectedItems.splice(action.payload.index, 1);
            delete state.prices[action.payload.id];
        },
        setPrice: (state: State, { payload: { id, value }}: PayloadAction<{id: string, value: string}>) => {
    ...
    extraReducers: (builder) => {
      ...
      .addCase(fetchRestaurants.fulfilled, (state: State, action: PayloadAction<Restaurant[]>) => {
      ...
      .addCase(fetchProducts.fulfilled, (state: State, { payload: { restaurantId, response }}: PayloadAction<{response: Product[], restaurantId: number}>) => {
