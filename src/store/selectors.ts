...
export const getPizzas = createSelector(getProducts, (items) =>
  items.filter((item: Product) => item.category === "Pizza")
);

export const getSummary = createSelector(getPrices, (prices) =>
  Object.values(prices).reduce(
    (partialSum: number, price: number) => partialSum + price,
    0
  )
);
